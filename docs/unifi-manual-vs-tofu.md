...
| # | Item | Where | Notes |
|---|---|---|---|
| A1 | **`tofu import` of every live resource, then a first `tofu plan`** | management plane `192.168.5.11`; `tofu/imports_unifi.tf` + [`unifi-import-plan.md`](unifi-import-plan.md) | **Prepared 2026-09-14, not yet run.** ⚠️ **CI plan run 34835272924 (2026-09-14, after the branch was pushed) failed at refresh**, exactly as the import plan's step 2 predicts: state still holds the pre-reset objects under the old submodule addresses (`module.unifi.module.{core,wireless,system,security}.*`) and the provider errors on each 404 (`Error Reading network` ×9, `Reading WLAN` ×4, VPN server, WAN, plus `Missing Resource Identity After Read` for the rest) instead of dropping them. **Run step 2 (`tofu state rm` of every `module.unifi.*` entry) on the management plane first**; the plan cannot get past refresh until then. The controller credential itself worked (no 403). The Proxmox and Vault resources refreshed fine. 46 `import` blocks with the live ids (one per real resource; the five commented-out blocks excluded), the id format per type, the state clean-up of the pre-reset entries, and a table of which first-plan diffs are expected versus a bug. Needs the management plane and an unsealed OpenBao. |
