...
| 3 | **`unifi_vpn_server` never reads `openvpn.mode` back** and **defaults `openvpn.encryption_cipher` to `AES_256_GCM`**, which the controller rejects on write. The controller stores neither field (verified live). | provider gap → **concession**: both in `ignore_changes` | `vpn.tf` FIXME |
