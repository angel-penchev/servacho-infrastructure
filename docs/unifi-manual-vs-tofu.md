...
| # | Item | Where | Notes |
|---|---|---|---|
| A1 | **`tofu import` of every live resource, then a first `tofu plan`** | management plane; `tofu/imports_unifi.tf` + [`unifi-import-plan.md`](unifi-import-plan.md) | 🔧 **Imports done** — apply run 34843335300 (2026-09-14) put all 46 objects in state and applied the guest `is_guest` and OpenVPN changes; two updates failed (§14.12 #11, #12). Since then: (a) the UniFi Devices network was normalised through the API (browser log, session (c)) — done; (b) the `unifi_setting` fill-in and the second apply — done (run 34843770282's predecessor). Remaining: (c) the one diff left is §14.12 #14 — `state rm module.unifi.unifi_network.unifi_devices` on the management plane, then one more apply re-imports it with `dhcp_guarding` populated; (d) plan must read **No changes**; (e) delete `tofu/imports_unifi.tf`. The `tofu-apply` environment has **no protection rules** — a dispatch applies immediately. |
