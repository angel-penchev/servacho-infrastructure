...
| # | Item | Where | Notes |
|---|---|---|---|
| A1 | **`tofu import` of every live resource, then a first `tofu plan`** | management plane; `tofu/imports_unifi.tf` + [`unifi-import-plan.md`](unifi-import-plan.md) | 🔧 **Imports done** — apply run 34843335300 (2026-09-14) put all 46 objects in state and applied the guest `is_guest` and OpenVPN changes; two updates failed (§14.12 #11, #12). Remaining: (a) the UI step on the UniFi Devices network described in the browser log, session (c); (b) re-run the apply for the `unifi_setting` fill-in; (c) plan must read **No changes**; (d) delete `tofu/imports_unifi.tf`. The `tofu-apply` environment has **no protection rules** — a dispatch applies immediately. |
...
| 13 | The OpenVPN update **wrote `openvpn_mode` and `openvpn_encryption_cipher` into the controller object** — keys the UI never creates. The controller accepted them and now stores them, so #3's perpetual `mode` diff may be gone. Watch the next plan. | side effect | `vpn.tf` FIXME |
