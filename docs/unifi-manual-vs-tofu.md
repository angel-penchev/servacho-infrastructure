...
| # | Item | Where | Notes |
|---|---|---|---|
| A1 | **`tofu import` of every live resource, then a first `tofu plan`** | management plane `192.168.5.11`; `tofu/imports_unifi.tf` + [`unifi-import-plan.md`](unifi-import-plan.md) | 🔧 **Plan clean as of 2026-09-14** after the state cleanup on the management plane (52 pre-reset entries removed) and two rounds of code corrections (commits `d05d9a7` and the one after it — see the file comments for each finding: Proxmox node name, `setting_preference`, DHCP guarding, `forget_on_destroy`, JetKVM `network_id`, OpenVPN cipher, WAN `enabled`, WLAN `group_rekey`/`no2ghz_oui`). CI plan runs 34838643746 / 34838813522: **46 to import, 4 to change**; the plan is **46 to import, 4 to change**, all intentional or unavoidable: `unifi_setting.default` fill-in (§14.12 #1), `unifi_network.unifi_devices` gets the UI defaults + DHCP guarding the API create skipped, `unifi_vpn_server.openvpn` pins `AES_256_CBC` and re-adds `mode` (§14.12 #3), `unifi_wlan.stkr_guest` restores `is_guest = true` (A8). **Apply not yet run** — dispatch `tofu-apply.yaml` on this branch; note the `tofu-apply` environment has **no protection rules**, so a dispatch with `auto_approve=false` still applies immediately. |
