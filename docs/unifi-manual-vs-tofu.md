...
| **Alarm Manager** (audited 2026-09-14) | 17 rules, all controller defaults from first boot, Notify → all admins, Always, push on / email off; no profiles, nothing custom | 🚫 UniFi OS alarm service (`/api/v2/alarms/…`), no provider resource — documented rule by rule as `FIXME(unifi)` in new `system/alarms.tf` | UI-only; defaults, so a reset reproduces them |
